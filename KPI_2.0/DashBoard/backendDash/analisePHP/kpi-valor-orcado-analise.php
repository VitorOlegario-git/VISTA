<?php
/**
 * KPI ANÁLISE 5: VALOR ORÇADO NA ANÁLISE
 * 
 * Retorna SUM(valor_orcamento) de analise_resumo.
 * Total de valor orçado gerado pela área de análise.
...
try {
    ...
    // Valor médio por equipamento analisado
    $sqlEquip = "
    ...
    $equipamentosAnalisados = (int)($rowEquip['total'] ?? 0);
    
    $valorMedioPorEquip = $equipamentosAnalisados > 0 
        ? number_format($valorAtual / $equipamentosAnalisados, 2, ',', '.') 
        : '0,00';
    ...
    $kpi = [
        ...
        'unidade' => 'R$',
        'titulo' => 'Valor Orçado na Análise',
        'periodo' => 'Período selecionado',
        'contexto' => 'Total orçado pela análise',
        'referencia' => [
        ...
        'detalhes' => [
            ...
            'equipamentos_analisados' => $equipamentosAnalisados
        ]
    ];
    ...
    error_log("Erro em kpi-valor-orcado-analise.php: " . $e->getMessage());
    enviarErro(500, 'Erro ao calcular valor orçado na análise');
} finally {
    if (isset($conn)) {
        $conn->close();
    }
