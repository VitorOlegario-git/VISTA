...
<script>
const STATUS_ORDER = [
    'aguardando_nf_retorno','aguardando_nf','aguardando_pg','descarte','em_anali
se','em_reparo','envio_analise','envio_cliente','envio_expedicao','estocado','inspecao_qualidade'                                                               ];
...
function renderFilters(counts){
    ...
    btnAll.innerHTML = `<span class="label">Total</span><span class="count">${to
tal}</span>`;                                                                       btnAll.onclick = ()=>{ activeFilter=null; document.getElementById('currentFi
lter').textContent='Todos'; renderTable(); updateActiveBtn(); };                    container.appendChild(btnAll);
    ...
    STATUS_ORDER.forEach(code=>{
        ...
        b.innerHTML = `<span class="label">${STATUS_LABELS[code]||code}</span><s
pan class="count">${c}</span>`;                                                         b.onclick = ()=>{ activeFilter = code; document.getElementById('currentF
ilter').textContent = STATUS_LABELS[code]||code; renderTable(); updateActiveBtn(); };                                                                                   container.appendChild(b);
    });
...
function updateActiveBtn(){
    document.querySelectorAll('.status-btn').forEach(b=>{
        if(b.dataset.status === activeFilter) b.classList.add('active'); else if
 (activeFilter===null && b.innerText.includes('Total')) b.classList.add('active'); else b.classList.remove('active');                                                });
}
...
function renderTable(){
    ...
    const filtered = allItems.filter(i=>{
        ...
        if(activeLocker && String(i.locker || '') !== String(activeLocker)) retu
rn false;                                                                               return true;
    ...
    filtered.forEach(row=>{
        ...
        tr.innerHTML = `
            <td data-label="Razão Social">${escapeHtml(row.razao_social||'—')}</
td>                                                                                         <td data-label="Nota Fiscal">${escapeHtml(row.nota_fiscal||'—')}</td
>                                                                                           <td data-label="Quantidade">${row.quantidade ?? row.qtd ?? '—'}</td>
            <td data-label="Armário">${row.locker ? `<span class="locker-badge">${escapeHtml(row.locker)}</span>` : '—'}</td>                                               <td data-label="Status">${STATUS_LABELS[row.status]||row.status}</td
>                                                                                           <td data-label="Confirmar"><div style="display:flex;gap:8px;align-it
ems:center">${activeFilter==='aguardando_pg'?`<select data-id="${row.id}" class="locker-select" style="min-width:84px">                                                         <option value="">—</option>
                <option value="1">1</option>
                <option value="2">2</option>
                <option value="3">3</option>
                <option value="4">4</option>
                <option value="5">5</option>
            </select>`: ''}<button class="confirm-btn" onclick="confirmItem(${ro
w.id || 0}, this)">Confirmar</button></div></td>                                        `;
...
async function loadData(){
    ...
    try{
        const res = await fetch('/router_public.php?url=inventario-api&action=li
st', {                                                                                      method: 'GET',
            credentials: 'include',
            cache: 'no-cache',
        ...
        // Expecting json.items with fields: cliente_nome (or razao_social), not
a_fiscal, quantidade, status, id                                                        allItems = (json.items || []).map(r=>({
...
';                                                                                  try{
        const fd = new FormData(); fd.append('resumo_id', resumoId); fd.append('
action','confirm');                                                                     const res = await fetch('/router_public.php?url=inventario-api', {method
:'POST', body:fd, credentials: 'include'});                                             const j = await res.json();
        if(j && j.success){
    ...
    setTimeout(()=>{ btn.disabled=false; btn.innerHTML = original; btn.style.bac
kground=''; },1500);                                                            }

async function assignLocker(resumoId, locker, selectEl){
    try{
        const fd = new FormData(); fd.append('resumo_id', resumoId); fd.append('
locker', locker);                                                                       const res = await fetch('/router_public.php?url=inventario-api&action=as
sign_locker', {method:'POST', body:fd, credentials:'include'});                         const j = await res.json();
        if(j && j.success){
            const it = allItems.find(x=>String(x.id) === String(resumoId)); if(i
t) it.locker = locker || null; renderFilters(counts); renderTable();                    } else {
...
if(btnNewRecord) btnNewRecord.addEventListener('click', (e)=>{ e.preventDefault(
); openPanelNew(); });                                                          const btnClosePanel = document.getElementById('btn-close-panel');
if(btnClosePanel) btnClosePanel.addEventListener('click', (e)=>{ e.preventDefaul
t(); closePanel(); });                                                          const panelOverlay = document.getElementById('panel-overlay');
if(panelOverlay) panelOverlay.addEventListener('click', ()=> closePanel());     

document.getElementById('cancelAdd').addEventListener('click', (e)=>{ e.preventD
efault(); closePanel(); });                                                     document.getElementById('submitAdd').addEventListener('click', async ()=>{      
    ...
    const data_ultimo_registro = document.getElementById('add_data_ultimo_regist
ro').value.trim();                                                                  const codigo_rastreio_entrada = document.getElementById('add_codigo_rastreio
_entrada').value.trim();                                                            const codigo_rastreio_envio = document.getElementById('add_codigo_rastreio_e
nvio').value.trim();                                                                const nota_fiscal_retorno = document.getElementById('add_nota_fiscal_retorno
').value.trim();                                                                    const numero_orcamento = document.getElementById('add_numero_orcamento').val
ue.trim();                                                                          const valor_orcamento = document.getElementById('add_valor_orcamento').value
.trim();                                                                            const setor = document.getElementById('add_setor').value.trim();
    const locker = document.getElementById('add_locker').value;
    const confirmado = document.getElementById('add_confirmado').checked ? 1 : 0
;                                                                                   if(!razao || !nf){ alert('Razão social e nota fiscal são obrigatórios'); ret
urn; }                                                                              try{
        ...
        const res = await fetch('/router_public.php?url=inventario-api&action=cr
eate_manual', {method:'POST', body:fd, credentials:'include'});                         const j = await res.json();
        if(j && j.success && j.item){ allItems.unshift(j.item); counts[j.item.st
atus] = (counts[j.item.status]||0)+1; renderFilters(counts); renderTable(); closePanel(); }                                                                             else alert('Erro ao criar remessa');
...
; } });                                                                         
// locker filter
document.getElementById('lockerFilter').addEventListener('change', (e)=>{ active
Locker = e.target.value || null; renderTable(); });                             // Ensure locker visibility is correct on load
updateLockerVisibility();
...
nnerHTML='';                                                                                    filtered.forEach(row=>{
    ...
    if(addCnpj){
        ...
        addCnpj.addEventListener('blur', async ()=>{
            ...
            if(cnpjVal.length !== 18) return; // CNPJ formatado tem 18 caractere
s                                                                                           try{
                const res = await fetch('/BackEnd/buscar_cliente.php', {        
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencode
d' },                                                                                               body: 'cnpj=' + encodeURIComponent(cnpjVal)
                });
                const json = await res.json();
